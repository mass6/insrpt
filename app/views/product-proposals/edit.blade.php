...
@section('page-head')
    @include('layouts.horizontal.partials._page-head', ['heading' => 'Proposal ID: <strong>' . $product_proposal->proposal_id . '</strong>', 'subheading' => '[Editing]'])
@stop
...
@section('content')
    ...
    @include('product-requests.partials._data', ['product_request' => $product_request, 'collapsed' => 0, 'panel_type' => 'info' ])
    @if($quotation)
        @include('quotations.partials._data')
    @endif

    {{ Form::model($product_proposal, ['route' => ['product-proposals.update', $product_proposal->id], 'method' => 'PATCH']) }}
